...
export default function ChatThread({ getUrl, postUrl, myRole }) {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined} keyboardVerticalOffset={90}>
          ...
          return (
            <View style={[styles.bubbleRow, { justifyContent: mine ? "flex-end" : "flex-start" }]}>
              <View style={[styles.bubble, mine ? styles.bubbleMine : styles.bubbleTheirs]}>
                {!!item.imageUrl && (
                  <Image source={{ uri: assetUrl(item.imageUrl) }} style={styles.image} />
                )}
                {!!item.text && (
      ...
      />
      {!!error && <Text style={{ color: colors.danger, fontSize: 12, paddingHorizontal: 16 }}>{error}</Text>}
      <View style={styles.inputRow}>
        <TouchableOpacity onPress={attachImage} style={styles.attachBtn} disabled={sending}>
...
const styles = StyleSheet.create({
  ...
  bubbleTheirs: { backgroundColor: colors.card, borderWidth: 1, borderColor: colors.line },
  bubbleText: { fontSize: 14.5, color: colors.ink },
  image: { width: 200, height: 160, borderRadius: 8, marginBottom: 6 },
  time: { fontSize: 10, color: colors.inkSoft, marginTop: 4, textAlign: "right" },
  inputRow: {
    flexDirection: "row", alignItems: "flex-end", padding: 10, borderTopWidth: 1, borderTopColor: colors.line,
    backgroundColor: colors.card,
  },
  attachBtn: { padding: 8 },
  input: {
    flex: 1, borderWidth: 1, borderColor: colors.line, borderRadius: radius.pill, paddingHorizontal: 14,
    paddingVertical: 8, marginHorizontal: 6, maxHeight: 100, fontSize: 14.5, color: colors.ink,
  },
  sendBtn: { backgroundColor: colors.gold, borderRadius: radius.pill, paddingHorizontal: 14, paddingVertical: 10 },
});
